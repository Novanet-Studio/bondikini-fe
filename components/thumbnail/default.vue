...
<template>
  <div class="md:flex md:flex-row-reverse md:gap-4">
      ...
      <div class="max-w-3xl md:max-w-16rem">
        ...
        >
          ...
          <div class="absolute -bottom-4 flex w-full justify-center z-8">
            <button
              class="w-13 h-13 rounded-full bg-color-3 text-white text-xs font-bold shadow-md"
            >
              ${{ product.price }}
            </button>
          </div>
    ...
    <!-- Thumbnail -->
    <div class="ps-product__variants max-w-48 mt-5 md:max-w-24 md:mt-0">
      <swiper-container
        class="swiper-thumbs"
        ...
        thumbs
        :direction="
          ['md', 'lg', 'xl', '2xl', '3xl'].includes(breakpoint!)
            ? 'vertical'
            : 'horizontal'
        "
      >
...
<style scoped>
  ...
  .swiper-thumbs {
    @apply h-[100px] box-border px-0 py-[10px] md:w-[300px] md:h-[400px] lg:w-[300px] lg:h-[400px];
  }
